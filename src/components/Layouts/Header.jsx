...
const Header = () => {
  ...
  useEffect(() => {
    ...
    const inputs = {units, policyIds: policyids, id: user._id}
      try {
          const res = await axios.put( assets &&`https://54.159.18.143:3000/api/users/${user._id}`, inputs)
  ...
  useEffect(() => {
    const getAddressInfo = async() => {
      ...
      try {
        const res = await axios.post(connected && "https://54.159.18.143:3000/api/users", profile);
