...
//   },[assets])
  ...
  useEffect(() => {
    ...
    // console.log(inputs)
      try {
          const res = await axios.put( assets &&` http://35.90.187.170/api/users/${user._id}`, inputs)
      }catch(err){
        console.log(err);
      }
    }
  ...
  useEffect(() => {
    const getAddressInfo = async() => {
      ...
      try {
        const res = await axios.post(connected && "http://35.90.187.170/api/users", profile);
